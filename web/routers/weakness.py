...
from web.common import (
    DEFAULT_MODEL,
    WEAKNESS_REPORT_LOCK,
    WEAKNESS_REPORT_PROGRESS,
    WEAKNESS_REPORT_TIMEOUT_SEC,
    _ensure_session_analysis_ready,
    _ensure_session_purpose,
    _get_cached_weakness_report,
    _get_interview_session_or_404,
    _get_login_user,
    _get_owned_interview_session,
    _get_question_analysis_progress,
    _get_session_recording_counts,
    _has_session_purpose,
    _invalidate_cached_weakness_report,
    _is_question_analysis_complete,
    _load_session_question_items,
    _purge_session_audio_files,
    _set_cached_weakness_report,
    _wait_for_question_analysis,
...
def _run_weakness_report_job(inter_id: int) -> None:
    ...
    try:
        ...
        for index, row in enumerate(rows, start=1):
            ...
            try:
                ...
                if progress.get("status") == "running":
                    remaining_timeout = WEAKNESS_REPORT_TIMEOUT_SEC - (time.monotonic() - job_started)
                    if _wait_for_question_analysis(
                        db=db,
                        inter_id=inter_id,
                        sel_id=sel_id,
                ...
                score_payload = calculate_speech_scores(
                    ...
                    row.qust_question_text,
                )
                upsert_speech_summary(db, sel_id, score_payload)
                upsert_speech_detail(db, sel_id, score_payload)
        ...
        if not failed:
            report = build_improvement_report(db, inter_id)
        ...
        _update_weakness_report_progress(
            ...
            ok=len(failed) == 0,
            message="리포트 준비 완료" if not failed else "일부 실패",
...
async def weakness_questions(request: Request, session_id: int, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(
        "weakness/questions.html",
        {
            ...
            "weakness_items": _load_session_question_items(db, session_id),
        },
    )
...
async def start_weakness_question_generation(
    ...
    _ensure_session_purpose(
        source,
        "DEFAULT",
        "기본 면접 결과에서만 생성 가능합니다.",
    )
    if source.inter_status != "DONE":
        raise HTTPException(status_code=409, detail="분석 완료 후 시도해 주세요.")
...
async def start_weakness_report_job(inter_id: int, db: Session = Depends(get_db)):
    ...
    if total == 0 or recorded < total:
        raise HTTPException(status_code=400, detail="모든 녹음을 완료해 주세요.")

    _invalidate_cached_weakness_report(inter_id)
    with WEAKNESS_REPORT_LOCK:
        if WEAKNESS_REPORT_PROGRESS.get(inter_id, {}).get("status") == "running":
...
async def get_weakness_report_progress(inter_id: int):
    with WEAKNESS_REPORT_LOCK:
        ...
        if progress.get("status") == "running" and (
            ...
            WEAKNESS_REPORT_PROGRESS[inter_id] = progress
...
async def weakness_report(request: Request, session_id: int, db: Session = Depends(get_db)):
    ...
    if cached is not None:
        return templates.TemplateResponse(
            ...
            {"request": request, "session_id": session_id, "report": cached},
        )

    _ensure_session_analysis_ready(db, session_id)
    report = build_improvement_report(db, session_id)
    _set_cached_weakness_report(session_id, report)
    return templates.TemplateResponse(
        "weakness/report.html",
        {"request": request, "session_id": session_id, "report": report},
    )
...
async def weakness_report_detail(
    ...
    session = _get_interview_session_or_404(db, session_id)
    _ensure_session_purpose(session, "WEAKNESS", "약점 보강 세션이 아닙니다.")
    if _get_cached_weakness_report(session_id) is None:
        raise HTTPException(status_code=409, detail="리포트 미준비")
    return templates.TemplateResponse(
        "weakness/report_detail.html",
        {
            "request": request,
            "session_id": session_id,
            "detail": build_improvement_report_detail(db, session_id, question_id)["detail"],
        },
    )
